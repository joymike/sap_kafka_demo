...
public class StepByStepServer {
    ...
    /**
     * This class provides the implementation for the function STFC_CONNECTION. You
     * will find the RFC-enabled function STFC_CONNECTION in almost any ABAP system.
     * The function is pretty simple - it has 1 input parameter and 2 output
     * parameter. The content of the input parameter REQUTEXT is copied to the
     * output parameter ECHOTEXT. The output parameter RESPTEXT is set to "Hello
     * World".
     */
    ...
    /**
     * First server example. At first we get an instance of the JCoServer through
     * JCoServerFactory. The requested instance will be created, or an existing one
     * will be returned if the instance was created before. It is not possible to
     * run more then one instance with a particular configuration. Then we register
     * the implementation for the function STFC_CONNECTION provided by class
     * StfcConnectionHandler through FunctionHandlerFactory provided by JCo. You are
     * free to write your own implementation JCoServerFunctionHandlerFactory, if you
     * need more than simple mapping between function name and java class
     * implementing the function. Now we can start the server instance. After a
     * while the JCo runtime opens the server connections. You may check the server
     ...
     * calls, calls executed BACKGROUND TASK. At first we write am implementation
     * for JCoServerTIDHandler interface. This implementation is registered by the
     * server instance and will be used for each call send in "background task".
     * Without such implementation JCo runtime deny any tRFC calls. See javadoc for
     * interface JCoServerTIDHandler for details.
     */
    ...
    /**
     * The following server example demonstrates how to develop a function module
     * available only on Java side. At first we create the respective function meta
     * data, because the function is not available in ABAP DDIC. Then the function
     * meta data is stored in a custom repository which is registered with the
     * server instance. Naturally we also need the implementation of the function -
     * see the class SetTraceHandler.
     * 
     ...
     * 
     * DATA trace_level TYPE N. DATA trace_path TYPE STRING. DATA msg(255) TYPE C.
     * 
     * trace_level = '5'. trace_path = '.'.
     * 
     * CALL FUNCTION 'JCO_SET_TRACE' destination 'JCO_SERVER' EXPORTING TRACE_LEVEL
     * = trace_level TRACE_PATH = trace_path EXCEPTIONS COMMUNICATION_FAILURE = 1
     * SYSTEM_FAILURE = 2 MESSAGE msg RESOURCE_FAILURE = 3 OTHERS = 4 . IF SY-SUBRC
     * <> 0. write: 'ERROR: ', SY-SUBRC, msg. ENDIF.
     */
